...
public class Level2Screen implements Screen {
  ...
  private void createRedBody() {
    ...
    bodyDef.position.set(
        viewport.getWorldWidth() / 4.5f,
        viewport.getWorldHeight() - 1); // Same position as chuckBody
    redBody = world.createBody(bodyDef);

    PolygonShape shape = new PolygonShape();
    float halfWidth = redBodyTexture.getWidth() / 2f / 100f / 30f;
    float halfHeight = redBodyTexture.getHeight() / 2f / 100f / 30f;
    ...
    Gdx.input.setInputProcessor(
        new InputAdapter() {
          ...
          public boolean touchDown(int screenX, int screenY, int pointer, int button) {
            ...
            //     if (mouseJoint == null) {
            //       mouseJointDef = new MouseJointDef();
            //       mouseJointDef.bodyA = dummyBody2;
          ...
          public boolean touchUp(int screenX, int screenY, int pointer, int button) {
            ...
            //         else if (mouseJoint != null && redBody != null) {
            //   // Calculate release velocity
            //   Vector2 releasePosition = viewport.unproject(new Vector2(screenX, screenY));
            //   Vector2 dragDirection =
            //       dragStart.sub(releasePosition); // Opposite direction of the drag
  ...
  public void render(float delta) {
    ...
    if (redBodyCreated) {
      ...
      float bodyWidth2 = (redBodyTexture.getWidth() / 100f / 30f) * 0.7f;
      float bodyHeight2 = (redBodyTexture.getHeight() / 100f / 30f) * 0.7f;
      batch.draw(
          redBodyTexture,
